...
module ApplicationHelper
  ...
  def avatar_thumb(length, person, avatar_html_options={})
    return "" if person.nil?
    link_to_unless(person.deleted?, image_tag(person.image.url(length), avatar_html_options), person)
  end
  ...
  def huge_avatar_thumb(person, options={})
    # FIXME! Need a new picture length: :large
    image_tag person.image.url(:medium), { :alt => person.name(@current_community) }.merge(options)
  end
  ...
  def self.random_sting(length=6)
    ...
    random_string = ""
    1.upto(length) { |i| random_string << chars[rand(chars.length-1)] }
  ...
  # Return the right notification "badge" length depending
